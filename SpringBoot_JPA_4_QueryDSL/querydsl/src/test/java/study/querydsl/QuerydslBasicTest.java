...
//        List<Object[]> resultList = em.createQuery("select m, t from Member m left join m.team t on t.name = 'teamA'").getResultList();
    ...
    @Test
    public void caseQuery2() throws Exception {
...
//                .getResultList();
    ...
    @Test
    public void findDtoByField2() throws Exception {

        QMember memberSub = new QMember("memberSub");

        List<UserDto> result = queryFactory
                .select(Projections.fields(UserDto.class,
                        member.username.as("name"),      //필드명이 다르면 에러
                        ExpressionUtils.as(JPAExpressions   //as사용위해서
                                .select(memberSub.age.max())
                                .from(memberSub), "age")
                ))
                .from(member)
